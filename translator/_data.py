...
def ListsToTensor(xs, vocab=None, local_vocabs=None, unk_rate=0.):
    ...
    for i, x in enumerate(xs):
        ...
        if local_vocabs is not None and not isinstance(x, list):
            assert False
        if 0:
            print ('x:')
            print (x)
            if vocab is not None:
                print('[vocab.token2idx(_) for _ in x]:')
                print ([vocab.token2idx(_) for _ in x])
                print('vocab.unk_idx:')
                print(vocab.unk_idx)
            print ('y:')
...
def _ListsToTensor(xs, vocab=None, local_vocabs=None, unk_rate=0.):
    if 0:
        print ('xs:')
        print (xs)
        print (len(xs))
    ...
    for x in xs:
        ...
        for z in y:
            ...
            if 0:
                ...
                print(z)
                assert False
            zs.append(vocab.token2idx([STR]+z+[END]) +
                      [vocab.padding_idx]*(max_string_len - len(z)))
        ys.append(zs)
        if 0:
            print (ys)
            assert False
...
def batchify(data, vocabs, unk_rate=0.):
    ...
    if 1:
        _conc = ListsToTensor(
            [[CLS]+x['concept'] for x in data], 
            vocabs['concept'], unk_rate=unk_rate)
    if 0:
        print ('_conc')
        print (_conc)
        print(_conc.shape)
    ...
    _conc_char = ListsofStringToTensor(
        [[CLS]+x['concept'] for x in data], 
        vocabs['concept_char'])
    if 0:
        print('_conc_char')
        print(_conc_char)
        print(_conc_char.shape)
        assert False
    ...
    all_relations[tuple([cls_idx])] = 0 # CLS to other nodes
    all_relations[tuple([rcls_idx])] = 1 # other nodes to CLS
    all_relations[tuple([self_idx])] = 2 # Self loop
    ...
    for bidx, x in enumerate(data):
        ...
        brs = [ [2]+[0]*(n) ] # from [CLS] to [CLS] (SEL) & other nodes (CLS)
        #print ('n:', n)
        for i in range(n):
            rs = [1] # from the node to CLS
            for j in range(n):
    ...
    if 0:
        for i, brs in enumerate(_relation_type):
            print (i, brs.shape)
        assert False
    ...
    if 0:
        print('_relation_type:')
        print(_relation_type.shape)
        assert False
    ...
    if 0:
        print('local_token2idx:')
        print(local_token2idx)
        #print('local_idx2token:')
        #print(local_idx2token)
        assert False
    ...
    if 0:
        print ('augmented_token:')
        print (augmented_token)
        print ("[ x['cp_seq'] for x in data]:")
        print ([ x['cp_seq'] for x in data])
        assert False

    _token_out = _ListsToTensor(augmented_token, vocabs['predictable_token'], local_token2idx)[1:]
    _cp_seq = _ListsToTensor([ x['cp_seq'] for x in data], vocabs['predictable_token'], local_token2idx)
    if 0:
        print ('_token_out:')
        print (_token_out)
        print ('_cp_seq:')
...
class DataLoader(object):
    ...
    def __iter__(self):
        ...
        if self.train:
            ...
            if 0:
                print (idx[:100])
                print([len(self.data[x]) for x in idx][:100])
                assert False
        ...
        for i in idx:
            if 0:
                print (i)
                print (self.data[i].graph.nodes)
                print (self.data[i].graph.edges)
                print(self.data[i].name2concept)
                print(len(self.data[i].name2concept))
                print(self.data[i].target)
                print(len(self.data[i].target))
        ...
        def work(graph):
            ...
            if 0:
                ...
                print (
                    np.amin(
                        list(
                            self.vocabs['concept']._token2idx.values())), 
                    np.amax(
                        list(
                            self.vocabs['concept']._token2idx.values())))
                ...
                print (
                    np.amin(
                        list(
                            self.vocabs['token']._token2idx.values())),
                    np.amax(
                        list(
                            self.vocabs['token']._token2idx.values())))

                print ('vocab:predictable_token:')
                print (
                    np.amin(
                        list(
                            self.vocabs['predictable_token']._token2idx.values())), 
                    np.amax(
                        list(
                            self.vocabs['predictable_token']._token2idx.values())))
                ...
                for tk in cp_seq:
                    ...
                    if tk in concept:
                        print (tk, 'in concept')
                    if tk in self.vocabs['predictable_token']._token2idx:
                        print (tk, 'in vocab:predictable_token')
                    if tk in self.vocabs['concept']._token2idx:
                        print (tk, 'in vocab:concept')
                    if tk in self.vocabs['token']._token2idx:
                        print (tk, 'in vocab:token')
                    if tk in token2idx:
                        print (tk, 'in token2idx')
                    print ('===')
                assert False                   
